...
class GhostKitTactical {
  ...
  initializeComponents() {
    // Load scripts dynamically with proper path handling
    // Get base path from current script URL
    const basePath = (() => {
      const scripts = document.getElementsByTagName('script');
      for (let i = 0; i < scripts.length; i++) {
        if (scripts[i].src.includes('ghostkit-tactical.js')) {
          return scripts[i].src.split('assets/javascripts/ghostkit-tactical.js')[0];
        }
      }
      return '';
    })();
    ...
    const scripts = [
      { src: basePath + 'assets/javascripts/matrix-enhanced.js', feature: 'matrix' },
      { src: basePath + 'assets/javascripts/terminal-demo.js', feature: 'terminal' },
      { src: basePath + 'assets/javascripts/cyber-visualizer.js', feature: 'visualizer' }
    ];
