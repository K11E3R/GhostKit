...
class SQLiScanner(WebScanner):
    ...
    def scan(self, crawled_data: Dict[str, Any] = None) -> List[Dict[str, Any]]:
        ...
        if not parameters_to_test:
            parameters_to_test = {"id", "user", "username", "password", "query", "q", "search", "keyword", "email"}
    ...
    def _test_time_based(self, parameter: str) -> None:
        ...
        for payload in self.time_payloads:
            ...
            if response_time > (baseline_time + 2.5):  # Look for delay of at least 2.5 seconds
                url_with_payload = f"{base_url}?{parameter}={urllib.parse.quote(payload)}"
                self.add_finding(
                    vulnerability_type="sql_injection",
                    url=url_with_payload,
    ...
    def _test_boolean_based(self, parameter: str) -> None:
        ...
        for i in range(0, len(self.boolean_payloads), 2):
            ...
            if abs(len(response_true.content) - len(response_false.content)) > 10:
                url_with_payload = f"{base_url}?{parameter}={urllib.parse.quote(true_payload)}"
                self.add_finding(
                    vulnerability_type="sql_injection",
                    url=url_with_payload,
                    parameter=parameter,
...
class CommandInjectionScanner(WebScanner):
    ...
    def scan(self, crawled_data: Dict[str, Any] = None) -> List[Dict[str, Any]]:
        ...
        # If no parameters found, test some common ones
        if not parameters_to_test:
            parameters_to_test = {"cmd", "command", "exec", "query", "q", "run", "execute", "ping", "host", "ip"}
    ...
    def _test_parameter(self, parameter: str) -> None:
        ...
        for payload in time_payloads:
            ...
            if response_time > (baseline_time + 4.5):  # Look for delay of at least 4.5 seconds
                url_with_payload = f"{base_url}?{parameter}={urllib.parse.quote(payload)}"
                self.add_finding(
                    vulnerability_type="command_injection",
                    url=url_with_payload,
...
class Module(BaseModule):
    ...
    def __init__(self):
        super().__init__()
        self.description = "Web injection vulnerability scanner for SQL and command injection"
    ...
    def _create_arg_parser(self) -> argparse.ArgumentParser:
        ...
        parser.add_argument("-t", "--type", choices=["sqli", "cmdi", "all"], default="all",
        ...
        parser.add_argument("--crawl", action="store_true", help="Crawl the target first to discover parameters")
        ...
        if parsed_args.crawl:
            if not WebCoreModule:
                ...
                if crawl_result.get("status") == "success" and "crawl_data" in crawl_result:
                    ...
                    print(f"[*] Crawl completed, discovered {len(crawled_data.get('parameters', []))} parameters")
