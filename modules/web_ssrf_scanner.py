...
class SSRFScanner(WebScanner):
    ...
    def scan(self, crawled_data: Dict[str, Any] = None) -> List[Dict[str, Any]]:
        ...
        if crawled_data and "forms" in crawled_data:
            for form_id, form_data in crawled_data["forms"].items():
                for input_data in form_data.get("inputs", []):
                    if "name" in input_data and input_data.get("type", "") in [
                        "text",
                        "url",
                        "hidden",
                    ]:
        ...
        test_params = list(prioritized_params) + [
            p for p in parameters_to_test if p not in prioritized_params
        ]
        ...
        callback_urls = [
            url.replace("RANDOM", callback_id) for url in self.callback_domains
        ]
    ...
    def _test_direct_ssrf(self, parameter: str) -> None:
        ...
        for payload in self.ssrf_payloads:
            ...
            if (
                abs(len(response.text) - len(baseline.text)) > 100
    ...
    def _test_blind_ssrf(
        self, parameter: str, callback_urls: List[str], callback_id: str
    ) -> None:
        ...
        for callback_url in callback_urls:
            ...
            callback_detected = (
                False  # In reality, would check if callback was received
            )

            if callback_detected:
                url_with_payload = (
                    f"{base_url}?{parameter}={urllib.parse.quote(callback_url)}"
                )
                self.add_finding(
...
class SSRFTester:
    ...
    def port_scan(
        self, target_host: str, start_port: int = 1, end_port: int = 1000
    ) -> Dict[str, Any]:
        ...
        with ThreadPoolExecutor(max_workers=20) as executor:
            ...
            futures = [
                executor.submit(test_port, port)
        ...
        return {"target": target_host, "open_ports": open_ports}
...
class Module(BaseModule):
    ...
    def __init__(self):
        ...
        self.description = (
            "Web SSRF Scanner for detecting Server-Side Request Forgery vulnerabilities"
        )
    ...
    def _create_arg_parser(self) -> argparse.ArgumentParser:
        ...
        parser.add_argument(
            "--proxy", help="Proxy to use (e.g., http://127.0.0.1:8080)"
        )
        parser.add_argument(
            "--cookies", help="Cookies to include (format: name1=value1;name2=value2)"
        )
        parser.add_argument(
            "--headers", help="Custom headers (format: name1:value1;name2:value2)"
        )
        return parser
    ...
    def run(self, args: List[str] = None) -> Dict[str, Any]:
        ...
        # Create web target
        target = WebTarget(
            url=args.url, headers=headers, cookies=cookies, proxy=args.proxy
        )
