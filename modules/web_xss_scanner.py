...
class XSSScanner(WebScanner):
    ...
    def __init__(self, target: WebTarget):
        ...
        self.dom_xss_payloads = [
            ...
            {"source": "location.search", "payload": "?xss=<img src=x onerror=alert(\'XSS\')>"},
            {"source": "document.referrer", "payload": "<img src=x onerror=alert(\'XSS\')>"},
            {"source": "document.cookie", "payload": "<img src=x onerror=alert(\'XSS\')>"}
        ]
    ...
    def scan(self, crawled_data: Dict[str, Any] = None) -> List[Dict[str, Any]]:
        ...
        if not parameters_to_test:
            parameters_to_test = {"q", "search", "query", "id", "name", "user", "input", "comment", "message", "content"}
            
        # If no URLs found, use the base URL
        if not urls_to_test:
            urls_to_test.add(self.target.base_url)
            
        print(f"[*] Testing {len(parameters_to_test)} parameters on {len(urls_to_test)} URLs for XSS")
        ...
        for payload in self.detection_payloads:
            ...
            if reflected:
                ...
                for bypass_payload in self.waf_bypass_payloads:
                    ...
                    if bypass_response and any(re.search(pattern, bypass_response.text, re.IGNORECASE) for pattern in self.reflection_patterns):
    ...
    def _analyze_html_context(self, html_content: str, payload: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            event_attrs = ["onclick", "onload", "onerror", "onmouseover", "onfocus", "onmouseout", "onkeypress"]
            for tag in soup.find_all():
            ...
            if marker in response.text:
                ...
                if contexts:
                    for context in contexts:
                        ...
                        context_payloads = self.context_payloads.get(context_type, self.context_payloads["html"])
                        ...
                        for ctx_payload in context_payloads:
                            ...
                            if ctx_response and any(re.search(pattern, ctx_response.text, re.IGNORECASE) for pattern in self.reflection_patterns):
                                self.add_finding(
                                    vulnerability_type="reflected_xss",
                                    url=form_url,
...
class Module(BaseModule):
    ...
    def _create_arg_parser(self) -> argparse.ArgumentParser:
        ...
        parser.add_argument("-c", "--cookies", help="Cookies to use in requests (format: name1=value1;name2=value2)")
        parser.add_argument("-H", "--headers", help="Custom headers (format: name1=value1;name2=value2)")
        parser.add_argument("-p", "--proxy", help="Proxy to use (format: http://host:port)")
        parser.add_argument("--timeout", type=int, default=10, help="Request timeout in seconds")
        parser.add_argument("--no-verify-ssl", action="store_true", help="Disable SSL verification")
        parser.add_argument("--crawl", action="store_true", help="Crawl the target first to discover parameters and forms")
        parser.add_argument("--crawl-depth", type=int, default=2, help="Maximum crawl depth")
        parser.add_argument("--dom-xss", action="store_true", help="Test for DOM-based XSS (may produce false positives)")
    ...
    def run(self, args: List[str]) -> Dict[str, Any]:
        ...
        if not WebTarget:
            self.logger.error("web_core module not found. This module requires web_core.py")
        ...
        if parsed_args.crawl:
            if not WebCoreModule:
                self.logger.warning("web_core module not properly imported, crawling disabled")
            else:
                print(f"[*] Crawling {parsed_args.url} to discover parameters and forms...")
                web_core = WebCoreModule()
                web_core_args = [
                    "--url", parsed_args.url,
                    "--action", "crawl",
                    "--depth", str(parsed_args.crawl_depth)
                ]
                ...
                if crawl_result.get("status") == "success" and "crawl_data" in crawl_result:
                    crawled_data = crawl_result["crawl_data"]
                    print(f"[*] Crawl completed, discovered {len(crawled_data.get('parameters', []))} parameters and {len(crawled_data.get('forms', {}))} forms")
        ...
        if findings:
            ...
            for i, finding in enumerate(findings[:5], 1):
                print(f"  {i}. {finding['type']} in {finding['parameter']} - {finding['severity'].upper()}")
